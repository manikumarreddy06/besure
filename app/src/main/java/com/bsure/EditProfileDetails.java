...
//    }
    ...
    public void updateUserAccount(UpdateUserAccountRequest updateUserAccountRequest){
        ...
        updateUserAccountResponseCall.enqueue(new Callback<UpdateUserAccountResponse>() {
            @Override
            public void onResponse(@NonNull Call<UpdateUserAccountResponse> call, @NonNull Response<UpdateUserAccountResponse> response) {
                UpdateUserAccountResponse z = response.body();
                if(response.isSuccessful()){
                    Toast.makeText(EditProfileDetails.this,"updated successfully ",Toast.LENGTH_LONG).show();
                    Log.i("Edit Profile", "Updated successfully ");
                }else{
                    Log.i("Edit Profile", "request failed");
                    Toast.makeText(EditProfileDetails.this,"request failed",Toast.LENGTH_LONG).show();
                }
            }

            @Override
            public void onFailure(@NonNull Call<UpdateUserAccountResponse> call, @NonNull Throwable t) {
                Toast.makeText(EditProfileDetails.this,"request failed here",Toast.LENGTH_LONG).show();
            }
        });
    }
}