...
//            startActivity(browserIntent)
                ...
                override fun onResponse(call: Call<signup?>, response: Response<signup?>) {
                    if (response.isSuccessful) {
                        ...
                        PreferenceManager.instance(this@Signin_Activity)
                            .set(PreferenceManager.USER_ID, response.body()!!.userId)

                        PreferenceManager.instance(this@Signin_Activity)
                            .set(PreferenceManager.DEVICE_TOKEN, response.body()!!.usmToken)
    ...
    fun verifyOtpApiCall() {
        val enterOtp =
            otp_edit_box1.text.toString() + otp_edit_box2.text.toString() + otp_edit_box3.text.toString() + otp_edit_box4.text.toString() + otp_edit_box5.text.toString()
        if (enterOtp.length == 5) {
            Utils.showDialog(Signin_Activity@ this, "loading")
            val obj = signup()
            obj.otp = enterOtp
            obj.userId = PreferenceManager.instance(this@Signin_Activity)
                .get(PreferenceManager.USER_ID, null)
            ...
            call.enqueue(object : Callback<BaseResponse?> {
                override fun onResponse(
                    ...
                    if (bean!!.isvalid) {
                        openMainActvity()
                        PreferenceManager.instance(this@Signin_Activity)
                            .set(PreferenceManager.LOGIN_STATUS, true)
                        val usernum = etmobile_signin.text.toString()

                        PreferenceManager.instance(this@Signin_Activity)
                            .set(PreferenceManager.USER_MOBILE_NUMBER, usernum)
    ...
    fun updatetokenrequest() {
        val obj1 = signupupdate()
    /*    obj1.userId = PreferenceManager.instance(this@Signin_Activity)
            .get(PreferenceManager.USER_ID, null)*/
            ...
            override fun onResponse(call: Call<signupupdate>, response: Response<signupupdate>) {
                if (response.isSuccessful) {
                    obj1.userId = PreferenceManager.instance(this@Signin_Activity).set(PreferenceManager.USER_ID, null)
                        .toString()
                    obj1.usmToken = PreferenceManager.instance(this@Signin_Activity).set(PreferenceManager.DEVICE_TOKEN,null)
                        .toString()
    ...
    private fun attachTextWatchers() {
        ...
        otp_edit_box5.addTextChangedListener(
            GenericTextWatcher(
                otp_edit_box5,
                null,
                Signin_Activity@ this
